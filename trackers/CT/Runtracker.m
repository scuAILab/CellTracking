...
% dataPath = '/home/jamin/Documents/MATLAB/cell/tracker/CT_code_v0';
caseID = 'test_';
imagePath = fullfile(dataPath,caseID);
%----------------------------------
% load init.txt;
% initstate = init;%initial tracker
%----------------------------Set path
img_dir = dir(fullfile(imagePath,'*.jpg'));
%---------------------------
img = imread(fullfile(imagePath ,img_dir(1).name));
img = double(img(:,:,1));
imshow (img,[]);
rect = getrect();
initstate = ceil(rect);
...
%--------------------------------------------------------
for i = 2:num
    img = imread(fullfile(imagePath ,img_dir(i).name));
